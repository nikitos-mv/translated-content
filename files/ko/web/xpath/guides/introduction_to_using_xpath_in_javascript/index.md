...
이 문서는 JavaScript 안, 확장기능, 웹사이트에서 [XPath](/ko/XPath)를 사용하기 위한 인터페이스를 설명합니다. Mozilla는 [DOM 3 XPath](https://www.w3.org/TR/DOM-Level-3-XPath/xpath.html)를 상당량 구현합니다. 이것은 XPath 식이 HTML과 XML 문서 모두에서 잘 돌아간다는 것을 뜻합니다.

XPath를 사용하는 주 인터페이스는 [document](/ko/docs/Web/API/Document) 개체의 [evaluate](/ko/docs/DOM/document.evaluate) 함수입니다.

## document.evaluate

이 메소드는 [XML](/ko/XML)에 기반을 둔 문서(HTML 문서 포함)에 대해 [XPath](/ko/XPath) 식을 평가하고 단일 노드나 노드 집합일 수 있는 `XPathResult` 개체를 반환합니다. 이 메소드를 다루는 기존 문서는 [여기](/ko/docs/DOM/document.evaluate)에 있지만 당장 우리 필요에 좀 부족하여, 더 이해가 빠른 설명을 아래에 제공합니다.
...
[evaluate](/ko/docs/DOM/document.evaluate) 함수는 매개변수를 총 5개 취합니다.
...
우리는 [document](/ko/docs/Web/API/Document) 개체의 `createNSResolver` 메소드를 써서 namespace resolver를 만듭니다.
...
우리가 문서 전체에서 heading을 찾기를 바라기 때문에, 우리는 `contextNode`로 [document](/ko/docs/Web/API/Document) 개체 자신을 씁니다.
...
확장기능 안에서 XML 문서의 컨텐트를 쓸 수 있게, 우리는 문서를 로드하는 동시에 변수 `xmlDoc`를 우리가 `evaluate` 메소드를 쓸 수 있는 [`XMLDocument`](/ko/XMLDocument) 개체로 문서에 포함하는 [`XMLHttpRequest`](/ko/docs/Web/API/XMLHttpRequest) 개체를 만듭니다,
...
`mathml:` 접두사를 namespace URI '[`http://www.w3.org/1998/Math/MathML`](https://www.w3.org/1998/Math/MathML)'와 `html:`을 URI [`http://www.w3.org/1999/xhtml`](https://www.w3.org/1999/xhtml)와 관련시키기 위해 우리는 함수 하나를 제공합니다.
...
- 원본 문서 [Mozilla XPath 입문서](https://www-xray.ast.cam.ac.uk/~jgraham/mozilla/xpath-tutorial.html)를 기반으로 함
